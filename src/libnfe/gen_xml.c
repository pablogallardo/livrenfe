...
int _gen_ide(xmlTextWriterPtr writer, NFE *nfe){
	...
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "cUF",
			"%d", nfe->idnfe->endereco->Mun->uf->cUF);
	if (rc < 0)
		return -EXML;
...
int _gen_emit(xmlTextWriterPtr writer, NFE *nfe){
	...
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "cPais",
			"%d", nfe->emitente->endereco->Mun->uf->pais->cPais);
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "xPais",
			"%s", nfe->emitente->endereco->Mun->uf->pais->xPais);
	if (rc < 0)
		return -EXML;
...
int _gen_dest(xmlTextWriterPtr writer, NFE *nfe){
	...
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "cMun",
			"%d", nfe->destinatario->endereco->Mun->cMun);
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "xMun",
			"%s", nfe->destinatario->endereco->Mun->xMun);
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "UF",
			"%s", nfe->destinatario->endereco->Mun->uf->xUF);
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "cPais",
			"%d", nfe->destinatario->endereco->Mun->uf->pais->cPais);
	if (rc < 0)
		return -EXML;
	rc = xmlTextWriterWriteFormatElement(writer, BAD_CAST "xPais",
			"%s", nfe->destinatario->endereco->Mun->uf->pais->xPais);
	if (rc < 0)
		return -EXML;
